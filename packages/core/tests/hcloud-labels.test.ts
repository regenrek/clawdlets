...
describe("hcloud-labels", () => {
  it("validates label keys/values", async () => {
    const { isValidHcloudLabelKey, isValidHcloudLabelValue } = await import("@clawdlets/cattle-core/lib/hcloud-labels");
  ...
  it("slugs unsafe values deterministically", async () => {
    const { toHcloudLabelValueSlug } = await import("@clawdlets/cattle-core/lib/hcloud-labels");
